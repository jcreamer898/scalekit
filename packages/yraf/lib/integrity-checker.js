...
export default class InstallationIntegrityChecker {
  ...
  _compareIntegrityFiles(actual, expected, checkFiles, workspaceLayout) {
    ...
    if (actual.flags.indexOf("checkFiles") === -1) {
      relevantExpectedFlags = relevantExpectedFlags.filter(
        (flag) => flag !== "checkFiles"
      );
    }
    ...
    if (
      !compareSortedArrays(
        actual.topLevelPatterns,
        expected.topLevelPatterns || []
      )
    ) {
    ...
    if (checkFiles) {
      ...
      for (let u = 0, v = 0; u < expected.files.length; ++u) {
        ...
        const max =
          v + (actual.files.length - v) - (expected.files.length - u) + 1;
  ...
  async check(patterns, lockfile, flags, workspaceLayout) {
    ...
    const missingPatterns = patterns.filter(
      (p) =>
        !lockfile[p] &&
        (!workspaceLayout || !workspaceLayout.getManifestByPattern(p))
    );
    ...
    if (integrityMatches === "OK") {
      invariant(
        expected,
        "The integrity shouldn't pass without integrity file"
      );
      for (const modulesFolder of expected.modulesFolders) {
        if (
          !(await fs.exists(
            path.join(this.config.lockfileFolder, modulesFolder)
          ))
        ) {

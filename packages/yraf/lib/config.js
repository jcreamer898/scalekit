...
export default class Config {
  ...
  async init(opts = {}) {
    ...
    if (
      this.focus &&
      (!this.workspaceRootFolder || this.cwd === this.workspaceRootFolder)
    ) {
    ...
    for (const key of Object.keys(registries)) {
      ...
      const extraneousRcFiles =
        Registry === registries.yarn ? this.extraneousYarnrcFiles : [];
    ...
    this.globalFolder =
      opts.globalFolder || String(this.getOption("global-folder", true));
    if (this.globalFolder === "undefined") {
      this.globalFolder = constants.GLOBAL_MODULE_DIRECTORY;
    }

    let cacheRootFolder =
      opts.cacheFolder || this.getOption("cache-folder", true);

    if (!cacheRootFolder) {
      let preferredCacheFolders = constants.PREFERRED_MODULE_CACHE_DIRECTORIES;
      const preferredCacheFolder =
        opts.preferredCacheFolder ||
        this.getOption("preferred-cache-folder", true);

      if (preferredCacheFolder) {
        preferredCacheFolders = [String(preferredCacheFolder)].concat(
      ...
      for (const skippedEntry of cacheFolderQuery.skipped) {
        this.reporter.warn(
          this.reporter.lang("cacheFolderSkipped", skippedEntry.folder)
        );
      }
      ...
      if (cacheRootFolder && cacheFolderQuery.skipped.length > 0) {
        this.reporter.warn(
          this.reporter.lang("cacheFolderSelected", cacheRootFolder)
        );
      }
    }
    ...
    this.ignoreScripts =
      opts.ignoreScripts || Boolean(this.getOption("ignore-scripts", false));

    this.workspacesEnabled =
      this.getOption("workspaces-experimental") !== false;
    this.workspacesNohoistEnabled =
      this.getOption("workspaces-nohoist-experimental") !== false;

    this.enableMetaFolder = Boolean(this.getOption("enable-meta-folder"));
    this.enableLockfileVersions = Boolean(
      this.getOption("yarn-enable-lockfile-versions")
    );
    this.linkFileDependencies = Boolean(
      this.getOption("yarn-link-file-dependencies")
    );
    this.packBuiltPackages = Boolean(
      this.getOption("experimental-pack-script-packages-in-mirror")
    );

    this.autoAddIntegrity = !boolifyWithDefault(
      String(this.getOption("unsafe-disable-integrity-migration")),
      true
    );
  ...
  _init(opts) {
    ...
    // $FlowFixMe$
    this.nonInteractive =
      !!opts.nonInteractive || isCi || !process.stdout.isTTY;
  ...
  executeLifecycleScript(commandName, cwd) {
    if (this.ignoreScripts) {
      ...
      /* MYS alwas hardcode ignoreScripts to true
  ...
  async readManifest(dir, priorityRegistry, isRoot = false) {
  ...
  async findWorkspaceRoot(initial) {
    ...
    do {
      ...
      if (ws && ws.packages) {
        ...
        if (
          relativePath === "" ||
          micromatch([relativePath], ws.packages).length > 0
        ) {
          return current;
        } else {
          return null;
  ...
  async resolveWorkspaces(root, rootManifest) {
    ...
    const ignorePatterns = this.registryFolders.map(
      (folder) => `/${folder}/**/+(${registryFilenames})`
    );
    ...
    for (const file of new Set([].concat(...files))) {
      ...
      if (!manifest.version) {
        this.reporter.warn(
          this.reporter.lang("workspaceVersionMandatory", loc)
        );
        continue;
      }
      ...
      if (Object.prototype.hasOwnProperty.call(workspaces, manifest.name)) {
        throw new MessageError(
          this.reporter.lang("workspaceNameDuplicate", manifest.name)
        );
      }
  ...
  getWorkspaces(manifest, shouldThrow = false) {
    ...
    if (wsCopy && wsCopy.nohoist && wsCopy.nohoist.length > 0) {
      if (!this.workspacesNohoistEnabled) {
        warnings.push(
          this.reporter.lang("workspacesNohoistDisabled", manifest.name)
        );

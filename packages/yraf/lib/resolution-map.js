...
import parsePackagePath, {
  isValidPackagePath,
} from "./util/parse-package-path";
...
export default class ResolutionMap {
  ...
  parsePatternInfo(globPattern, range) {
    if (!isValidPackagePath(globPattern)) {
      this.reporter.warn(
        this.reporter.lang("invalidResolutionName", globPattern)
      );
      return null;
    }
  ...
  find(reqPattern, parentNames) {
    ...
    const { pattern, range } =
      resolutions.find(({ globPattern }) =>
        minimatch(modulePath, globPattern)
      ) || {};

    if (pattern) {
      if (
        semver.validRange(reqRange) &&
        semver.valid(range) &&
        !semver.satisfies(range, reqRange)
      ) {
        // At this point, it seems moot to keep this in here...
        // this.reporter.warn(
        //   this.reporter.lang(

...
export async function copyBulk(queue, reporter, _events) {
  /*
   * The code here has been removed as an ongoing effort to remove
   * dead code one small and safe step at the time.
   */
}

export async function hardlinkBulk(queue, reporter, _events) {
  const events = {
  ...
  const actions = await buildActionsForHardlink(
    queue,
    events,

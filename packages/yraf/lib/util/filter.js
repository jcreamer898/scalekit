  ...
  return (
    lines
      ...
      .map((line) => {
        ...
        const regex = mm.makeRe(pattern.trim(), {
          dot: true,
          nocase: true,
        });
...
export function filterOverridenGitignores(files) {
  ...
  return files
    .filter((file) => IGNORE_FILENAMES.indexOf(file.basename) > -1)
    .reduce((acc, file) => {
      if (file.basename !== GITIGNORE_NAME) {
        ...
        const higherPriorityIgnoreFilePaths = [
          path.join(dir, IGNORE_FILENAMES[0]),
          path.join(dir, IGNORE_FILENAMES[1]),
        ];
        const hasHigherPriorityFiles = files.find(
          (file) =>
            higherPriorityIgnoreFilePaths.indexOf(
              path.normalize(file.absolute)
            ) > -1
        );

...
export default class NpmResolver extends RegistryResolver {
  static async findVersionInRegistryResponse(
    ...
    if (body.versions && Object.keys(body.versions).length === 0) {
      throw new MessageError(
        config.reporter.lang("registryNoVersions", body.name),
      );
    }

    if (!body["dist-tags"] || !body.versions) {
      throw new MessageError(
        config.reporter.lang("malformedRegistryResponse", name),
      );
    }
    ...
    const latestVersion = body["dist-tags"]
    ...
    throw new MessageError(
      config.reporter.lang(
        ...
        body.name,
        range,
        semver.rsort(Object.keys(body.versions)).join("\n"),
      ),
    );
  }
  ...
  async resolve() {
    ...
    if (shrunk) {
      if (
        this.config.packBuiltPackages &&
        shrunk.prebuiltVariants &&
        shrunk._remote
      ) {

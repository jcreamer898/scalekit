...
export default class PackageResolver {
  ...
  addPattern(pattern, info) {
    ...
    const byName = (this.patternsByPackage[info.name] =
      this.patternsByPackage[info.name] || []);
  ...
  getExactVersionMatch(name, version, manifest) {
    ...
    if (manifest && getExoticResolver(version)) {
      return this.exoticRangeMatch(
        patterns.map(this.getStrictResolvedPattern.bind(this)),
        manifest,
      );
    }
  ...
  getHighestRangeVersionMatch(name, range, manifest) {
    ...
    if (!maxValidRange) {
      return manifest && getExoticResolver(range)
        ? this.exoticRangeMatch(resolvedPatterns, manifest)
        : null;
    }
  ...
  exoticRangeMatch(resolvedPkgs, manifest) {
    ...
    const matchedPkg = resolvedPkgs.find(
      ({ _remote: pkgRemote }) =>
        pkgRemote &&
        pkgRemote.reference === remote.reference &&
        pkgRemote.type === "copy",
    );
  ...
  async find(initialReq, extraDependencies) {
  ...
  async init(
    deps,
    { isFlat, isFrozen, workspaceLayout, extraDependencies } = {
      isFlat: false,
      isFrozen: false,
  ...
  optimizeResolutions(name) {
    ...
    const collapsablePatterns = patterns.filter((pattern) => {
      ...
      return (
        !this.lockfile.getLocked(pattern) &&
        (!remote || remote.type !== "workspace")
      );
    });
    ...
    const availableVersions = this.getAllInfoForPatterns(
      collapsablePatterns,
    ).map((manifest) => manifest.version);
    availableVersions.sort(semver.rcompare);

    const ranges = collapsablePatterns.map(
      (pattern) => normalizePattern(pattern).range,
    );
  ...
  resolveToResolution(req) {
    ...
    if (resolution) {
      ...
      if (resolutionManifest) {
        invariant(
          resolutionManifest._reference,
          "resolutions should have a resolved reference",
        );

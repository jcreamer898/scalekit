...
export default class Git {
  ...
  static async secureGitUrl(ref, hash, reporter) {
    ...
    if (ref.protocol === "git:") {
      ...
      if (await Git.repoExists(secureUrl)) {
        ...
        reporter.warn(
          reporter.lang("downloadGitWithoutCommit", ref.repository),
        );
        return ref;
      }
    }
    ...
    if (ref.protocol === "http:") {
      ...
      if (await Git.repoExists(secureRef)) {
        ...
        reporter.warn(
          reporter.lang("downloadHTTPWithoutCommit", ref.repository),
        );
        return ref;
      }
    }
  ...
  async setRefRemote() {
    ...
    if (isLocal) {
      stdout = await spawnGit(["show-ref", "--tags", "--heads"], {
        cwd: this.gitUrl.repository,
      });
    } else {
      stdout = await spawnGit([
        "ls-remote",
        "--tags",
        "--heads",
  ...
  async resolveDefaultBranch() {
    ...
    try {
      let stdout;
      if (isLocal) {
        stdout = await spawnGit(["show-ref", "HEAD"], {
          cwd: this.gitUrl.repository,
        });
        const refs = parseRefs(stdout);

...
export function checkOne(info, config, ignoreEngines) {
  ...
  if (
    shouldCheckCpu(cpu, config.ignorePlatform, config.ignoreCpu) &&
    !isValidArch(cpu)
  ) {
    ...
    for (const entry of entries(info.engines)) {
      ...
      if (VERSIONS[name]) {
        if (!testEngine(name, range, VERSIONS, config.looseSemver)) {
          pushError(
            reporter.lang("incompatibleEngine", name, range, VERSIONS[name]),
          );
        }
...
export function shouldCheck(manifest, options) {
  return (

...
const PATH_CONFIG_OPTIONS = new Set([
  "cache",
  "cafile",
  "prefix",
  "userconfig",
...
export default class NpmRegistry extends Registry {
  ...
  getRequestUrl(registry, pathname) {
    ...
    if (!REGEX_REGISTRY_PREFIX.test(pathname)) {
      resolved = urlResolve(addSuffix(registry, "/"), pathname);
    }
  ...
  getParams(pathname, opts = {}, packageName) {
    ...
    return this.requestManager.getParams({
      url: requestUrl,
      method: opts.method,
      body: opts.body,
      auth: opts.auth,
  ...
  async request(pathname, opts = {}, packageName) {
    ...
    try {
      ...
      if (error instanceof OneTimePasswordError) {
        ...
        this.reporter.info(
          this.reporter.lang("twoFactorAuthenticationEnabled"),
        );
        this.otp = this.reporter.question(
          this.reporter.lang("npmOneTimePassword"),
        );
